...
An [MCP](https://modelcontextprotocol.io) (Model Context Protocol) server that lets an AI harness (Claude, or any MCP-compatible client) control [Recordly](https://github.com/webadderallorg/Recordly) — screen recording, editor state, and (eventually) export — end to end.

This package is the **client half** of a two-part system:

1. **Automation Server** (`electron/automation/`, lives in the main Recordly repo) — a localhost-only, token-authed WebSocket JSON-RPC server that runs *inside* Recordly's Electron main process. It's opt-in only: a normal manual `npm run dev`/packaged launch never starts it. It only activates when `RECORDLY_MCP_TOKEN` is set.
2. **This package** — a standalone MCP stdio server. It either attaches to an already-running Recordly instance (via a lockfile) or spawns one itself (`npm run dev` in the Recordly repo, with a generated token), then translates MCP tool calls into JSON-RPC calls against the automation server.
...
| Tool | What it does |
|---|---|
| `get_app_status` | Whether Recordly is currently recording, and the platform |
...
| `list_projects` | List saved `.recordly` project files |
| `read_project` | Read a `.recordly` project file's contents |
| `open_editor` | Open (or focus) Recordly's editor window |
...
mcp-server/src/connection.ts  (attach-or-launch)
   ...
   ▼
electron/automation/server.ts  (inside Recordly's Electron main process)
   │
...
Two different bridging mechanisms exist because Recordly's own architecture has two different places state can live:
- Recording/project-file operations are handled entirely in the **main process** (native capture helpers, filesystem), so the automation server can call the same internal handler functions directly.
...
- The automation server **never starts** unless `RECORDLY_MCP_TOKEN` is explicitly set — a normal manual Recordly launch is completely unaffected.
- This package and Recordly must run on the same machine. There is no remote-control mode.
...
- **No caption tools yet.** `generate_captions` would be an async, potentially long-running Whisper transcription call — a different shape than the synchronous tools shipped so far.
...
- **Can't open a specific project by path into the editor.** There's no "editor ready with project X loaded" signal in Recordly yet; tools assume you're operating on whatever's already open.
...
- **A pre-existing, intermittent native recording flake** ("moov atom not found" on `stop_recording` right after a pause→resume) has been observed a few times during live testing — it's in Recordly's existing ScreenCaptureKit pipeline, unrelated to this MCP work. See `SMOKE_TEST.md` for details.

## License

This package is part of the Recordly repository and inherits its **AGPL-3.0** license (see the root `LICENSE.md`). If you fork or redistribute this work, the same reciprocal-source and attribution terms apply.
