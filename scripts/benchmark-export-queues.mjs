...
const width = parseEvenInteger(process.env.FRAMEWRIGHT_BENCH_EXPORT_WIDTH ?? "1280", "Width");
const height = parseEvenInteger(process.env.FRAMEWRIGHT_BENCH_EXPORT_HEIGHT ?? "720", "Height");
const frameRate = parsePositiveInteger(process.env.FRAMEWRIGHT_BENCH_EXPORT_FPS ?? "60", "Frame rate");
const durationSeconds = parsePositiveInteger(
	process.env.FRAMEWRIGHT_BENCH_EXPORT_DURATION ?? "15",
	"Duration",
);
const timeoutMs = parsePositiveInteger(
	process.env.FRAMEWRIGHT_BENCH_EXPORT_TIMEOUT_MS ?? "180000",
	"Timeout",
);
const runsPerVariant = parsePositiveInteger(process.env.FRAMEWRIGHT_BENCH_EXPORT_RUNS ?? "2", "Runs");
const useNativeExport = process.env.FRAMEWRIGHT_BENCH_EXPORT_USE_NATIVE === "1";
const useWebcamOverlay = process.env.FRAMEWRIGHT_BENCH_EXPORT_ENABLE_WEBCAM === "1";
const providedInputPath = process.env.FRAMEWRIGHT_BENCH_EXPORT_INPUT ?? null;
const providedWebcamInputPath = process.env.FRAMEWRIGHT_BENCH_EXPORT_WEBCAM_INPUT ?? null;
const keepTempArtifacts = process.env.FRAMEWRIGHT_BENCH_EXPORT_KEEP_TEMP === "1";
const exportEncodingMode = parseExportEncodingMode(
	process.env.FRAMEWRIGHT_BENCH_EXPORT_ENCODING_MODE ?? null,
);
const exportQuality = parseExportQuality(process.env.FRAMEWRIGHT_BENCH_EXPORT_QUALITY ?? null);
const exportShadowIntensity = parseExportShadowIntensity(
	process.env.FRAMEWRIGHT_BENCH_EXPORT_SHADOW_INTENSITY ?? null,
);
const webcamWidth = parseEvenInteger(
	process.env.FRAMEWRIGHT_BENCH_EXPORT_WEBCAM_WIDTH ?? "640",
	"Webcam width",
);
const webcamHeight = parseEvenInteger(
	process.env.FRAMEWRIGHT_BENCH_EXPORT_WEBCAM_HEIGHT ?? "360",
	"Webcam height",
);
const webcamShadowIntensity = parseExportShadowIntensity(
	process.env.FRAMEWRIGHT_BENCH_EXPORT_WEBCAM_SHADOW ?? null,
);
const webcamSize = parseExportWebcamSize(process.env.FRAMEWRIGHT_BENCH_EXPORT_WEBCAM_SIZE ?? null);
const MODERN_BACKEND_SWEEP = ["auto", "webcodecs", "breeze"];
const exportPipeline = parseExportPipeline(process.env.FRAMEWRIGHT_BENCH_EXPORT_PIPELINE ?? null);
const exportBackend = parseExportBackend(process.env.FRAMEWRIGHT_BENCH_EXPORT_BACKEND ?? null);
const exportRenderBackend = parseRenderBackend(
	process.env.FRAMEWRIGHT_BENCH_EXPORT_RENDER_BACKEND ?? null,
);
const exportBackendList = parseExportBackendList(
	process.env.FRAMEWRIGHT_BENCH_EXPORT_BACKENDS ?? null,
);
...
const variantNameList = parseBenchmarkVariantList(
	process.env.FRAMEWRIGHT_BENCH_EXPORT_VARIANTS ?? null,
);
...
function parseExportPipeline(rawValue) {
	...
	throw new Error("FRAMEWRIGHT_BENCH_EXPORT_PIPELINE must be 'legacy' or 'modern'");
}
...
function parseExportBackend(rawValue) {
	...
	throw new Error("FRAMEWRIGHT_BENCH_EXPORT_BACKEND must be 'auto', 'webcodecs', or 'breeze'");
}
...
function parseRenderBackend(rawValue) {
	...
	throw new Error("FRAMEWRIGHT_BENCH_EXPORT_RENDER_BACKEND must be 'webgl' or 'webgpu'");
}
...
function parseExportBackendList(rawValue) {
	...
	if (values.length === 0) {
		throw new Error(
			"FRAMEWRIGHT_BENCH_EXPORT_BACKENDS must include at least one of: auto, webcodecs, breeze",
		);
	}
...
function parseBenchmarkVariantList(rawValue) {
	...
	if (values.length === 0) {
		throw new Error(
			"FRAMEWRIGHT_BENCH_EXPORT_VARIANTS must include at least one of: adaptive, baseline, tuned",
		);
	}
	...
	for (const value of values) {
		if (!(value in VARIANT_PRESETS)) {
			throw new Error(
				"FRAMEWRIGHT_BENCH_EXPORT_VARIANTS must include only: adaptive, baseline, tuned",
			);
		}
	}
...
function parseExportEncodingMode(rawValue) {
	...
	throw new Error("FRAMEWRIGHT_BENCH_EXPORT_ENCODING_MODE must be 'fast', 'balanced', or 'quality'");
}
...
function parseExportQuality(rawValue) {
	...
	throw new Error("FRAMEWRIGHT_BENCH_EXPORT_QUALITY must be 'medium', 'good', 'high', or 'source'");
}
...
function parseExportShadowIntensity(rawValue) {
	...
	if (!Number.isFinite(parsed) || parsed < 0) {
		throw new Error("FRAMEWRIGHT_BENCH_EXPORT_SHADOW_INTENSITY must be a non-negative number");
	}
...
function parseExportWebcamSize(rawValue) {
	...
	if (!Number.isFinite(parsed) || parsed <= 0 || parsed > 100) {
		throw new Error("FRAMEWRIGHT_BENCH_EXPORT_WEBCAM_SIZE must be a number between 0 and 100");
	}
...
async function runVariant(
	...
	const child = spawn(electron, [repoRoot], {
		...
		env: {
			...process.env,
			FRAMEWRIGHT_SMOKE_EXPORT: "1",
			FRAMEWRIGHT_SMOKE_EXPORT_INPUT: inputPath,
			FRAMEWRIGHT_SMOKE_EXPORT_OUTPUT: outputPath,
			...(useNativeExport ? { FRAMEWRIGHT_SMOKE_EXPORT_USE_NATIVE: "1" } : {}),
			...(exportEncodingMode
				? { FRAMEWRIGHT_SMOKE_EXPORT_ENCODING_MODE: exportEncodingMode }
				: {}),
			...(exportQuality ? { FRAMEWRIGHT_SMOKE_EXPORT_QUALITY: exportQuality } : {}),
			...(exportShadowIntensity !== null
				? { FRAMEWRIGHT_SMOKE_EXPORT_SHADOW_INTENSITY: String(exportShadowIntensity) }
				: {}),
			...(webcamInputPath ? { FRAMEWRIGHT_SMOKE_EXPORT_WEBCAM_INPUT: webcamInputPath } : {}),
			...(webcamShadowIntensity !== null
				? { FRAMEWRIGHT_SMOKE_EXPORT_WEBCAM_SHADOW: String(webcamShadowIntensity) }
				: {}),
			...(webcamSize !== null
				? { FRAMEWRIGHT_SMOKE_EXPORT_WEBCAM_SIZE: String(webcamSize) }
				: {}),
			...(benchmarkRequest.pipeline
				? { FRAMEWRIGHT_SMOKE_EXPORT_PIPELINE: benchmarkRequest.pipeline }
				: {}),
			...(benchmarkRequest.backend
				? { FRAMEWRIGHT_SMOKE_EXPORT_BACKEND: benchmarkRequest.backend }
				: {}),
			...(exportRenderBackend
				? { FRAMEWRIGHT_SMOKE_EXPORT_RENDER_BACKEND: exportRenderBackend }
				: {}),
			...(typeof variant.maxEncodeQueue === "number"
				? { FRAMEWRIGHT_SMOKE_EXPORT_MAX_ENCODE_QUEUE: String(variant.maxEncodeQueue) }
				: {}),
			...(typeof variant.maxDecodeQueue === "number"
				? { FRAMEWRIGHT_SMOKE_EXPORT_MAX_DECODE_QUEUE: String(variant.maxDecodeQueue) }
				: {}),
			...(typeof variant.maxPendingFrames === "number"
				? { FRAMEWRIGHT_SMOKE_EXPORT_MAX_PENDING_FRAMES: String(variant.maxPendingFrames) }
				: {}),
		},
