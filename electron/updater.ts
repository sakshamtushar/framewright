...
const AUTO_UPDATES_DISABLED = process.env.FRAMEWRIGHT_DISABLE_AUTO_UPDATES === "1";
const UPDATE_FEED_URL_OVERRIDE = process.env.FRAMEWRIGHT_UPDATE_FEED_URL?.trim() ?? "";
const UPDATER_LOG_PATH =
	process.env.FRAMEWRIGHT_UPDATER_LOG_PATH?.trim() || path.join(USER_DATA_PATH, "updater.log");
	...
	return {
		...
		detail:
			normalizedProgress >= 100
				? "Finishing the update download. Framewright will restart as soon as the installer is ready."
				: remainingMb !== null
					? `${remainingMb.toFixed(1)} MB left before Framewright restarts.`
					: "Downloading the update now. Framewright will restart when it finishes.",
		delayMs: UPDATE_REMINDER_DELAY_MS,
...
export async function downloadAvailableUpdate(
	...
	setUpdateStatusSummary({
		status: "downloading",
		availableVersion,
		detail: `Downloading Framewright ${availableVersion}`,
	});
...
async function showAvailableUpdateDialog(
	...
	const result = await showMessageBox(getMainWindow, {
		...
		message: `Framewright ${version} is available.`,
...
async function showDownloadedUpdateDialog(
	...
	const result = await showMessageBox(getMainWindow, {
		...
		title: "Update Ready",
		message: isPreview
			? `Framewright ${version} is ready to install.`
			: `Framewright ${version} has been downloaded.`,
		detail: isPreview
		...
		if (options?.manual) {
			await showMessageBox(getMainWindow, {
				...
				detail: AUTO_UPDATES_DISABLED
					? "This build disabled auto-updates through FRAMEWRIGHT_DISABLE_AUTO_UPDATES=1."
...
export function setupAutoUpdates(
	...
	autoUpdater.on("update-available", (info) => {
		...
		setUpdateStatusSummary({
			status: "available",
			availableVersion: info.version,
			detail: `Framewright ${info.version} is available.`,
		});
	...
	autoUpdater.on("update-not-available", () => {
		...
		setUpdateStatusSummary({
			status: "up-to-date",
			availableVersion: null,
			detail: `Framewright ${app.getVersion()} is up to date.`,
		});
		clearVisibleUpdateToast(sendToRenderer);
		manualCheckRequested = false;
	});
	...
	autoUpdater.on("update-downloaded", (info) => {
		...
		setUpdateStatusSummary({
			status: "ready",
			availableVersion: info.version,
			detail: `Framewright ${info.version} is ready to install.`,
		});
		clearDeferredReminderTimer();
