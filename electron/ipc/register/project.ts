...
export function registerProjectHandlers() {
  ...
  handle('save-project-file', async (_, projectData: unknown, suggestedName?: string, existingProjectPath?: string, thumbnailDataUrl?: string | null) => {
    try {
      ...
      const result = await dialog.showSaveDialog({
        title: 'Save Framewright Project',
        defaultPath: path.join(projectsDir, defaultName),
        filters: [
          { name: 'Framewright Project', extensions: [PROJECT_FILE_EXTENSION] },
  ...
  handle('load-project-file', async () => {
    try {
      const projectsDir = await getProjectsDir()
      const result = await dialog.showOpenDialog({
        title: 'Open Framewright Project',
        defaultPath: projectsDir,
        filters: [
          { name: 'Framewright Project', extensions: [PROJECT_FILE_EXTENSION, ...LEGACY_PROJECT_FILE_EXTENSIONS] },
          { name: 'JSON', extensions: ['json'] },
