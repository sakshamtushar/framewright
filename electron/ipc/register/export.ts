...
function getPartialExportDestinationPath(destinationPath: string) {
	...
	return path.join(parsed.dir, `.framewright-partial-${parsed.name}-${suffix}${parsed.ext}`);
}
	...
	ipcMain.handle(
		...
		async (
			...
			try {
				...
				const sessionId = `framewright-export-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
