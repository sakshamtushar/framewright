...
export function KeyboardShortcutsHelp() {
	...
	return (
		<div className="relative group">
			<HelpCircle className="w-4 h-4 text-muted-foreground/70 hover:text-[#E5AF89] transition-colors cursor-help" />
			...
			<div className="absolute right-0 top-full mt-2 w-64 bg-editor-dialog border border-foreground/10 rounded-lg p-3 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 shadow-xl z-50">
				<div className="flex items-center justify-between mb-2">
					...
					<button
						...
						className="flex items-center gap-1 text-[10px] text-muted-foreground/70 hover:text-[#E5AF89] transition-colors"
					>
				...
				<div className="space-y-1.5 text-[10px]">
					{SHORTCUT_ACTIONS.map((action) => (
						<div key={action} className="flex items-center justify-between">
							...
							<kbd className="px-1 py-0.5 bg-foreground/5 border border-foreground/10 rounded text-[#E5AF89] font-mono">
