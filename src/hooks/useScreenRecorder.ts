...
export function useScreenRecorder(): UseScreenRecorderReturn {
	...
	const preparePermissions = useCallback(async (options: { startup?: boolean } = {}) => {
		...
		if (!screenPermission.success || screenPermission.status !== "granted") {
			...
			alert(
				options.startup
					? "Framewright needs Screen Recording permission before you start. System Settings has been opened. After enabling it, quit and reopen Framewright."
					: "Screen Recording permission is still missing. System Settings has been opened again. Enable it, then quit and reopen Framewright before recording.",
			);
			return false;
		}
		...
		alert(
			options.startup
				? "Framewright also needs Accessibility permission for cursor tracking. System Settings has been opened. After enabling it, quit and reopen Framewright."
				: "Accessibility permission is still missing. System Settings has been opened again. Enable it, then quit and reopen Framewright before recording.",
		);
