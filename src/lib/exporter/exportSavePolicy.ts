...
export function describeBlockedInMemoryExportSave({
	...
	if (isExportTooLargeForInMemorySave(blobSize)) {
		return `The ${normalizedExtension.toUpperCase()} export is too large to save through the legacy in-memory path. Please retry the export so Framewright can save it through the temp-file streaming path.`;
	}

	return `The ${normalizedExtension.toUpperCase()} export could not be saved through the temp-file streaming path, and Framewright will not fall back to the legacy in-memory path for MP4 exports. Please retry the export.`;
}
